...
Gift-voucher spend (GYFTR/Amazon Pay/Swiggy/Instamart) never touches a real bank account, so it's otherwise invisible to Munim — undercounting Groceries/Dining for anything routed through a voucher. This feature pulls the purchase and redemption detail straight out of Gmail and represents it as a virtual `voucher-<brand>` `Assets` account per brand, using the same double-entry pattern as a manually-tracked asset like a house purchase: a real voucher purchase becomes a credit into the account (linked to its funding bank transaction), and each real-world redemption becomes a debit out of it in the transaction's actual category. Three commands, run in order: `munim-ingest gmail fetch-vouchers` fetches the four email types into a local file (same Gmail app-password handling as `gmail fetch` — a user-run terminal command only); `munim vouchers import <file>` turns those records into transactions; `munim vouchers recheck` re-evaluates every redemption against current bank data. That last one matters because the redemption-vs-already-real-spend decision depends on the corresponding bank statement already being imported — **import a period's bank statement before running voucher import for that period** — and `recheck` is the self-correcting fix if that order gets reversed (e.g. voucher import ran ahead of that period's card statement). See `munim-ingest gmail fetch-vouchers --help` and `munim vouchers --help`.
